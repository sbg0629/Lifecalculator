...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'dday' });
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-8 mb-8 transition-colors">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-gray-100 mb-4 transition-colors">
            {locale === 'ko' ? 'D-Day 계산기 안내' : 'D-Day Calculator Guide'}
          </h2>
          <div className="prose max-w-none text-gray-700 dark:text-gray-300 space-y-4 transition-colors">
            {locale === 'ko' ? (
              <p>
                D-Day 계산기는 특정 날짜까지 남은 일수를 계산해드리는 서비스입니다. 기념일, 생일, 중요한 날짜까지 남은 일수를 확인할 수 있습니다.
              </p>
            ) : (
              <p>
                The D-Day calculator calculates the days remaining until a specific date. You can check the days remaining until anniversaries, birthdays, and important dates.
              </p>
            )}
          </div>
        </div>
