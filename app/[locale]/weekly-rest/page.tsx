...
import WeeklyRestCalculator from '@/components/WeeklyRestCalculator';
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'weeklyRest' });
...
export default async function WeeklyRestPage({
  params
}: {
  params: Promise<{ locale: string }>;
}) {
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-8 mb-8 transition-colors">
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-gray-100 mb-4 transition-colors">
            {locale === 'ko' ? '주휴수당 계산기 안내' : 'Weekly Rest Pay Calculator Guide'}
          </h2>
          <div className="prose max-w-none text-gray-700 dark:text-gray-300 space-y-4 transition-colors">
            {locale === 'ko' ? (
              <p>
                주휴수당 계산기는 주 40시간 근무 시 주휴수당을 계산해드리는 서비스입니다. (2026년 기준, 근로기준법)
              </p>
            ) : (
              <p>
                The weekly rest pay calculator calculates weekly rest pay for 40-hour work week. (2026 standard, Labor Standards Act)
              </p>
            )}
          </div>
        </div>
